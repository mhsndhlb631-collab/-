...
4. Generate an independent 24+ character `RUNTIME_DATABASE_PASSWORD`, put it only in ignored `.env.local`, and run `npm run db:setup-runtime`. This creates the fixed `tarbiyah_app_runtime` LOGIN with NOSUPERUSER/NOBYPASSRLS/NOCREATEROLE/NOCREATEDB/NOINHERIT and membership only in the limited `tarbiyah_runtime` capability role. It owns no app tables. Every data transaction must issue fixed `SET LOCAL ROLE tarbiyah_runtime`; the login otherwise has no inherited domain access and the role cannot leak across transactions. Build DATABASE_URL from the Transaction pooler URI (port 6543), replacing only the username with `tarbiyah_app_runtime.<project-ref>` and using this independent password. Never grant the runtime login migrator membership.
