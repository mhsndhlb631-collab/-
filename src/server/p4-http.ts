import "server-only";
import { P4LearningService } from "../application/p4-learning-service";
...
export function p4Query(
  request: Request,
  action: (service: P4LearningService) => Promise<unknown>,
) {
  return endpoint("p4-query", (requestId) =>
    withAuthenticatedTransaction(request, (tx, actor) =>
      action(new P4LearningService(tx, actor, requestId)),
    ),
  );
}
export function p4Command(
  request: Request,
  action: (
    service: P4LearningService,
    body: unknown,
    key: string | null,
  ) => Promise<unknown>,
) {
  return endpoint("p4-command", async (requestId) => {
    ...
    try {
      return await withAuthenticatedTransaction(request, (tx, actor) =>
        action(
          new P4LearningService(tx, actor, requestId),
          body,
