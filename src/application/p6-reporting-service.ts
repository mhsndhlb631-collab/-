...
export class P6ReportingService {
  ...
  async studentWeek(query: unknown) {
    ...
    const rows = await this.tx`select sp.id,p.display_name,
      ...
      (select count(*)::int from app.assignment_targets t join app.assignment_occurrences o on o.assignment_definition_id=t.assignment_definition_id where t.enrollment_id=e.id and o.due_at::date between ${from}::date and ${to}::date) operational_assignments_due,
      (select count(*)::int from app.assignment_evaluations av join app.assignment_occurrences o on o.id=av.occurrence_id where av.enrollment_id=e.id and o.due_at::date between ${from}::date and ${to}::date) operational_assignments_evaluated,
      (select round(avg(av.normalized_score)*100,1) from app.assignment_evaluations av join app.assignment_occurrences o on o.id=av.occurrence_id where av.enrollment_id=e.id and o.due_at::date between ${from}::date and ${to}::date and av.normalized_score is not null) operational_assignment_average,
  ...
  async groupWeek(query: unknown) {
    ...
    const rows = await this.tx`select g.id,g.name,
      ...
      (select count(*)::int from app.assignment_occurrences o join app.assignment_definitions d on d.id=o.assignment_definition_id where d.group_id=g.id and o.due_at::date between ${from}::date and ${to}::date) operational_assignment_occurrences,
      (select count(*)::int from app.assignment_evaluations av join app.assignment_occurrences o on o.id=av.occurrence_id join app.assignment_definitions d on d.id=o.assignment_definition_id where d.group_id=g.id and o.due_at::date between ${from}::date and ${to}::date) operational_assignment_evaluations,
      (select round(avg(av.normalized_score)*100,1) from app.assignment_evaluations av join app.assignment_occurrences o on o.id=av.occurrence_id join app.assignment_definitions d on d.id=o.assignment_definition_id where d.group_id=g.id and o.due_at::date between ${from}::date and ${to}::date and av.normalized_score is not null) operational_assignment_average,
  ...
  async program(query: unknown) {
    ...
    const rows = await this.tx`select c.id,c.name,
      ...
      (select count(*)::int from app.assignment_occurrences o join app.assignment_definitions d on d.id=o.assignment_definition_id join app.groups g on g.id=d.group_id where g.cohort_id=c.id and o.due_at::date between ${from}::date and ${to}::date) operational_assignment_occurrences,
      (select count(*)::int from app.assignment_evaluations av join app.assignment_occurrences o on o.id=av.occurrence_id join app.assignment_definitions d on d.id=o.assignment_definition_id join app.groups g on g.id=d.group_id where g.cohort_id=c.id and o.due_at::date between ${from}::date and ${to}::date) operational_assignment_evaluations,
      (select round(avg(av.normalized_score)*100,1) from app.assignment_evaluations av join app.assignment_occurrences o on o.id=av.occurrence_id join app.assignment_definitions d on d.id=o.assignment_definition_id join app.groups g on g.id=d.group_id where g.cohort_id=c.id and o.due_at::date between ${from}::date and ${to}::date and av.normalized_score is not null) operational_assignment_average,
